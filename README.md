...
- **Active console detection with localized spoken warnings** — the service only counts usage for the active console session, launches one `activityhelper.exe` per active user, and delivers English or Czech startup, halfway, five-minute, and countdown announcements over a private live HTTP stream.
...
## Scheduled Days Mode
Scheduled-days mode uses the normal daily allowance on enabled weekdays and blocks use entirely on disabled weekdays. On disabled days, the helper announces that use is unavailable and waits for `reenforcement_delay_sec` before countdown and hibernation. It is the default installer mode, and can also be selected explicitly with `-QuotaMode scheduled-days` or `quota_mode` set to `scheduled-days` in config. Administrators configure `enabled_weekdays` as seven booleans ordered Monday through Sunday; the default is `[true,true,true,true,true,false,false]`. The installer also accepts `-EnabledWeekdays` with seven boolean values in the same order.
